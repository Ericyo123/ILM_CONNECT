...
function LecturerMessagesContent() {
  const { user } = useAuth();
  ...
  // Automatically fade out unread indicators once lecturer is viewing with active tab
  ...
  const handleStartWithStudent = (student: Student) => {
    ...
    handleSelectThread(syntheticThread);
  };
  ...
  // Merge threads with any students not yet in threads
  const studentsInThreads = new Set(threads.map((t) => t.otherUser.id));
  const studentsWithoutThreads = myStudents.filter((s) => !studentsInThreads.has(s.userId));
  ...
  // Account role mismatch check (e.g. student browsing lecturer page)
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] p-6 text-center animate-fade-in">
        <div className="p-6 rounded-2xl bg-amber-500/10 border border-amber-500/30 max-w-md shadow-lg">
          ...
          <h2 className="text-lg font-bold mb-1">Student Account Active</h2>
          <p className="text-xs text-[hsl(var(--muted-foreground))] mb-4 leading-relaxed">
            You are currently signed in as <span className="font-semibold text-[hsl(var(--foreground))]">{user.email}</span> (Student). To view the Lecturer Portal, please sign in with your lecturer account (e.g. in an Incognito window for pair-testing).
          </p>
  ...
  // Calculate unread items for active thread (incoming messages from student)
  const activeUnreadCount = messages.filter(
    (m) => (m.sender?.role === 'STUDENT' || m.recipientId === user.id) && (!m.readAt || sessionUnreadIds.has(m.id))
  ).length;
    ...
    <div className="flex h-[calc(100vh-4rem)] -m-4 lg:-m-6 overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          ) : threads.length === 0 && studentsWithoutThreads.length > 0 ? (
            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
              <MessageSquare className="h-10 w-10 text-[hsl(var(--muted-foreground)/0.3)] mb-3" />
              <p className="font-semibold text-sm mb-1">No conversations yet</p>
              <p className="text-xs text-[hsl(var(--muted-foreground))]">
                Click &quot;New Chat&quot; to message one of your students
              </p>
            </div>
          ) : threads.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
        ...
        {!selectedThread ? (
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
            ...
            <p className="text-sm text-[hsl(var(--muted-foreground))] max-w-xs">
              Select a student conversation or start a new one using the &quot;New Chat&quot; button.
            </p>
          </div>
        ) : (
          <>
              ...
              {messagesLoading ? (
                ...
                <AnimatePresence initial={false}>
                  {messages.map((msg, idx) => {
                    ...
                    const isMine = !isFromStudent && (msg.sender?.role === 'LECTURER' || msg.senderId === user.id);
                    ...
                    return (
                      <div key={msg.id}>
                        ...
                        >
                          {!isMine && (
                            <div className="h-7 w-7 rounded-full bg-[hsl(var(--primary)/0.15)] flex items-center justify-center text-[hsl(var(--primary))] font-bold text-[10px] flex-shrink-0 mr-2 mt-1">
                              {selectedThread.otherUser.initials}
                            </div>
                          )}
